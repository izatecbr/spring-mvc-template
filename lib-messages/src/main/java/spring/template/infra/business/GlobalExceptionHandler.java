...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    private ResponseEntity<Object> handleGeneral(Exception e, WebRequest request) {
        ...
        Response error = ResponseFactory.error(HttpStatus.INTERNAL_SERVER_ERROR.value(), be.getMessage(), be.getSuggestion());
    ...
    private ResponseEntity<Object> handleBusinessException(BusinessException be, WebRequest request) {
        ...
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        return handleExceptionInternal(be, error, headers, HttpStatus.resolve(be.getHttpStatus()), request);
    }
}
